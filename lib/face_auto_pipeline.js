...
// 连拍位置先验（2026-07-15）：同相册整图 resnet50 余弦≥0.6 的"连拍帧"里，位置相近的脸大概率同人，
// 可把单脸相似度门槛从 matchThreshold(0.36) 安全降到 0.30。
// ⚠️ 参数是全库网格验证的零矛盾工作点(142 个已确认样本)：裸先验矛盾率 38%(邻座互指/同场景换位)，
// 必须三条件齐用：imgCos≥0.6 + 中心距≤0.2 + 单脸sim≥0.30。放宽任何一条前先重跑验证。
const BURST_PRIOR_ENABLED = String(process.env.FACE_BURST_PRIOR || '1') !== '0';
...
async function loadBurstSiblingFaces(conn, photoId, projectId) {
  if (!BURST_PRIOR_ENABLED) return [];
  ...
  try {
    ...
    const [faceRows] = await conn.query(
      `SELECT photo_id AS photoId, person_id AS personId,
              ...
              normalized_embedding AS ne, embedding AS e
       FROM photo_faces WHERE photo_id IN (?) AND person_id IS NOT NULL`,
      [Array.from(sibCos.keys())]
    );
    const out = [];
...
async function detectAndClusterPhoto({
  ...
  try {
    ...
    // 连拍位置先验的上下文（无 embedding/无连拍邻居时为 []，先验自动不生效）
    const burstSiblings = faces.length ? await loadBurstSiblingFaces(conn, pid, photo.projectId) : [];
    ...
    for (let faceIndex = 0; faceIndex < faces.length; faceIndex++) {
      ...
      // 常规匹配不够强时，试连拍位置先验（验证过的三条件，见文件头常量注释）
      let burstMatch = null;
      if (!strongAssigned && burstSiblings.length && face.normalizedVector) {
