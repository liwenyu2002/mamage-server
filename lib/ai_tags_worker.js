  ...
  try {
    ...
    if (!originalRel || !thumbRel) {
      try {
        ...
        if (rows && rows[0]) {
          isVideo = isVideo || String(rows[0].type || '').toLowerCase() === 'video';
          if (!originalRel) originalRel = isVideo ? rows[0].thumb_url : (rows[0].url || rows[0].thumb_url);
    ...
    // 视频只分析转码流程产出的 JPEG 封面帧；原始视频不是视觉模型的图片输入。
    const analysisRel = isVideo ? thumbRel : (originalRel || thumbRel);
...
function _drain() {
  ...
  // 正常照片任务永远压过视频封面帧任务；视频只利用空闲窗口，不能拖慢新上传照片。
  ...
  try {
    ...
    const [rows] = await pool.query(
      `SELECT id, thumb_url AS thumbRel, url, type
       ...
       WHERE (
           COALESCE(type, '') <> 'video'
           OR (thumb_url IS NOT NULL AND thumb_url <> '')
         )
         AND (
