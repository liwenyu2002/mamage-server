...
const { transcribeVideoAudio } = require('./video_transcription');
...
async function captureFrame(input, time, targetPath) {
  const frameWidth = Math.round(numberEnv('VIDEO_SEMANTIC_FRAME_WIDTH', 960, 480, 1920));
  await runFfmpeg([
    '-hide_banner', '-loglevel', 'error', '-y', '-ss', String(Math.max(0, time)), '-i', input,
    '-frames:v', '1', '-vf', `scale=${frameWidth}:-2`, '-q:v', '3', targetPath,
  ], 90000);
}
...
async function buildStoryboard(input, sampleTimes, frameDir, segmentIndex) {
  ...
  try {
    // Preserve enough pixels for signs, screens and object labels. The semantic
    // prompt relies on these facts later when it creates the detailed timeline.
    const panelWidth = Math.round(numberEnv('VIDEO_SEMANTIC_PANEL_WIDTH', 448, 256, 768));
    const panelHeight = Math.round(numberEnv('VIDEO_SEMANTIC_PANEL_HEIGHT', 280, 160, 480));
    const gap = 8;
...
function selectedGlobalEvidenceSegments(segments) {
  const limit = Math.round(numberEnv('VIDEO_SEMANTIC_GLOBAL_EVIDENCE_FRAMES', 3, 1, 6));
  if (!segments.length) return [];
  if (segments.length <= limit) return segments.slice();

  const selected = new Map();
  (segments || []).filter((segment) => segment.keyMoment).forEach((segment) => {
    if (selected.size < limit) selected.set(segment.index, segment);
  });
  for (let index = 0; selected.size < limit && index < limit; index += 1) {
    const sourceIndex = Math.round((index / Math.max(1, limit - 1)) * (segments.length - 1));
    const segment = segments[sourceIndex];
    selected.set(segment.index, segment);
  }
  return Array.from(selected.values()).sort((left, right) => left.start - right.start).slice(0, limit);
}

async function buildGlobalEvidenceBoard(input, segments, frameDir) {
  const selected = selectedGlobalEvidenceSegments(segments);
  ...
  for (let index = 0; index < selected.length; index += 1) {
    ...
    const time = Number((segment.sampleTimes || [])[Math.max(0, (segment.sampleTimes || []).length - 1)]);
    if (!Number.isFinite(time)) continue;
    const framePath = path.join(frameDir, `global-evidence-${index}.jpg`);
    try {
      await captureFrame(input, time, framePath);
      paths.push(framePath);
      sampleTimes.push(time);
    } catch (error) {
      console.warn('[video-analysis] global evidence frame failed:', rounded(time), error && error.message ? error.message : error);
    }
  }
  if (!paths.length) return null;

  try {
    const panelWidth = Math.round(numberEnv('VIDEO_SEMANTIC_GLOBAL_EVIDENCE_WIDTH', 960, 640, 1440));
    const panelHeight = Math.round(numberEnv('VIDEO_SEMANTIC_GLOBAL_EVIDENCE_HEIGHT', 540, 360, 960));
    const gap = 10;
    const panels = await Promise.all(paths.map((framePath) => sharp(framePath)
      .rotate()
      .resize(panelWidth, panelHeight, { fit: 'contain', background: { r: 14, g: 18, b: 26, alpha: 1 } })
    ...
    const board = await sharp({
      create: {
        width: panelWidth,
        height: panels.length * panelHeight + Math.max(0, panels.length - 1) * gap,
        channels: 3,
        background: { r: 14, g: 18, b: 26 },
      },
    }).composite(panels.map((buffer, index) => ({ input: buffer, left: 0, top: index * (panelHeight + gap) })))
      .jpeg({ quality: 93, mozjpeg: true })
      .toBuffer();
    return { board, sampleTimes };
  } finally {
    await Promise.all(paths.map((framePath) => fs.promises.rm(framePath, { force: true }).catch(() => null)));
  }
}
...
async function analyzeVideo(input, duration = 0, options = {}) {
  ...
  let globalEvidence = null;
  let globalEvidenceFrameCount = 0;
  let transcript = sourceHasAudio
    ? { status: options.includeTranscript === false ? 'disabled' : 'pending', segments: [], text: '' }
  ...
  try {
    ...
    for (const scene of scenes) {
      const segment = {
        ...scene,
        sampleTimes: sampleTimesForSegment(scene.start, scene.end),
        summary: null,
        scene: null,
        eventStage: null,
        tags: [],
      ...
      if (semanticEnabled) {
        ...
        try {
          ...
          segment.scene = result.scene || null;
          segment.eventStage = result.eventStage || null;
          segment.tags = result.tags || [];
          segment.keyObjects = result.keyObjects || [];
          segment.visibleText = result.visibleText || [];
    ...
    if (semanticEnabled && segments.length) {
      try {
        const evidenceBoard = await buildGlobalEvidenceBoard(input, segments, frameDir);
        if (evidenceBoard) {
          globalEvidenceFrameCount = evidenceBoard.sampleTimes.length;
          const evidenceResult = await analyzeGlobalEvidenceBoard(evidenceBoard.board, {
            duration: resolvedDuration,
            sampleTimes: evidenceBoard.sampleTimes,
          });
          const { raw, ...persistedEvidence } = evidenceResult;
          globalEvidence = persistedEvidence;
        }
      } catch (error) {
        console.warn('[video-analysis] global evidence pass failed:', error && error.message ? error.message : error);
      }
    }
    if (sourceHasAudio && options.includeAudio !== false && options.includeTranscript !== false) {
      await notifyProgress(options.onProgress, { phase: 'transcript', completed: 0, total: 1 });
      transcript = await transcribeVideoAudio(input, resolvedDuration, { workDir: temporaryRoot });
      await notifyProgress(options.onProgress, { phase: 'transcript', completed: 1, total: 1, transcript });
    }
  } finally {
  ...
  const global = await summarizeTemporalTimeline({
    segments,
    duration: resolvedDuration,
    hasAudio: sourceHasAudio && silenceResult.mode !== 'unavailable' && silenceResult.mode !== 'no-audio-scan',
  ...
  return {
    ...
    method: 'temporal-storyboards-audio-transcript-and-global-evidence',
    semanticStatus,
    coverage: {
      start: 0,
      ...
      transcriptCoverage: rounded(transcript.coverage && transcript.coverage.duration),
    ...
    globalEvidence,
    global,
    totalSemantic: global.totalSemantic || global.detailedSummary || global.summary || '',
    detailedSummary: global.totalSemantic || global.detailedSummary || global.summary || '',
