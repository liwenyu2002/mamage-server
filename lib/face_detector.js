...
const { buildUploadUrl, buildInternalMediaUrl } = require('../db');
...
async function loadPhotoBinary(photoRow) {
  ...
  let resp;
  try {
    resp = await axios.get(buildInternalMediaUrl(url), {
...
async function writeTempImage(buffer, extHint) {
...
function runDetectorProcess({ python, script, tmpPath, timeoutMs }) {
  return new Promise((resolve, reject) => {
    ...
    let child;
    try {
      child = spawn(python, [script, tmpPath], {
        stdio: ['ignore', 'pipe', 'pipe'],
        windowsHide: true,
      });
    } catch (err) {
      done(reject, makeFaceDetectError(
    ...
    child.on('close', (code) => {
      if (timedOut) {
          ...
          'FACE_DETECT_TIMEOUT',
          'face detector timeout',
          `timeout=${timeoutMs}ms`
        ));
        return;
      }
      done(resolve, { status: code, stdout, stderr });
    });
  });
}
...
async function detectFacesForPhoto(photoRow) {
  ...
  const tmpPath = await writeTempImage(loaded.buffer, loaded.extHint);
  let stdout = '';
  let stderr = '';

  try {
    const res = await runDetectorProcess({ python, script, tmpPath, timeoutMs });
