...
async function aiFillArticleTemplate(slots, brief, opts = {}) {
  ...
  const system = [
    '你是高校公众号的资深推文创作者。用户给出一篇推文模板的占位符清单（带编号）和活动简报，你负责为每个占位符撰写成稿文案。',
    '只输出一个 JSON 对象，不要解释，不要 markdown 代码块。结构固定为：',
    '{"title": "推文标题（20字内，可带主副题分隔）", "digest": "摘要（54字内，概括亮点）", "values": ["第1个占位符的文案", "第2个占位符的文案", ...]}',
    'values 必须是与占位符清单等长、同顺序的字符串数组——不要输出对象、不要输出编号说明。',
    '写作要求：',
  ...
  const user = [
    ...
    '占位符清单（values 数组按此顺序输出）：',
    ...safeSlots.map((s, i) => `${i + 1}. ${s.label}${s.key !== s.label ? `（${s.key}）` : ''}${s.hint ? ` | 补充说明: ${s.hint}` : ''}`),
  ].filter(Boolean).join('\n');
  ...
  // 数组按下标对齐（首选）；对象按键名精确/宽松匹配兜底（历史格式/模型自由发挥）
  const normKey = (k) => String(k || '').replace(/[\s：:]/g, '').toLowerCase();
  const keyMap = new Map(safeSlots.map((s) => [normKey(s.key), s.key]));
  const values = {};
  if (Array.isArray(parsed.values)) {
    safeSlots.forEach((s, i) => {
      const v = parsed.values[i];
      if (v != null && String(v).trim()) values[s.key] = String(v);
    });
  } else if (parsed.values && typeof parsed.values === 'object') {
    for (const [rk, rv] of Object.entries(parsed.values)) {
      const key = keyMap.get(normKey(rk));
      if (key && rv != null && String(rv).trim() && values[key] == null) values[key] = String(rv);
    }
  }
  return {
