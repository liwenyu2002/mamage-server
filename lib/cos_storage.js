...
function ensureClient() {
  ...
  // 部分校内 S3/Ceph RGW 使用自签名或与 IP 不匹配的 HTTPS 证书。
  // 只允许为对象存储客户端关闭校验，绝不修改 NODE_TLS_REJECT_UNAUTHORIZED，
  // 避免削弱同一 Node 进程中的 AI、邮件等其他 HTTPS 请求。
  if (parseBoolean(process.env.COS_TLS_REJECT_UNAUTHORIZED) === false) {
...
async function signedPost(key, options = {}) {
  const s3 = ensureClient();
  if (!s3) throw new Error('S3 client not configured');
  const normalized = normalizeKey(key);
  if (!isSafeKey(normalized)) throw new Error('Invalid object key');
  const contentType = options.contentType || 'application/octet-stream';
  const cacheControl = options.cacheControl || process.env.UPLOAD_CACHE_CONTROL || 'public, max-age=31536000, immutable';
  const expiresIn = Math.max(60, Number(options.expires || process.env.COS_SIGNED_UPLOAD_EXPIRES_SECONDS || 900));
  const maxBytes = Math.max(1, Number(options.maxBytes) || 5 * 1024 * 1024 * 1024);
  const result = await createPresignedPost(s3, {
    Bucket: getBucket(),
    Key: normalized,
    Expires: expiresIn,
    Fields: { 'Content-Type': contentType, 'Cache-Control': cacheControl },
    Conditions: [
  ...
  return { key: normalized, postUrl: result.url, fields: result.fields, publicUrl: objectUrlForKey(normalized), expiresIn };
}
...
// 服务端代浏览器上传一个 multipart 分片（代理直传通道用）：
// 密钥仍只在服务端，浏览器把分片 PUT 到同源 API，由这里转存进对象存储。
async function uploadPart(key, uploadId, partNumber, body, options = {}) {
  const s3 = ensureClient();
  if (!s3) throw new Error('S3 client not configured');
  ...
  if (!result || !result.ETag) throw new Error('S3 part upload did not return ETag');
...
function deleteObjectsForPhotoRows(rows) {
  ...
  for (const row of rows || []) {
    ...
    const publicDownloadKey = keyFromUrlOrPath(row.publicDownloadUrl || row.public_download_url);
    ...
    if (publicDownloadKey) keysToDelete.push(publicDownloadKey);
