...
async function buildRenderSpec(project, assetRows, options = {}) {
  ...
  for (const clip of rawClips) {
    if (clip && clip.kind === 'blank') {
      ...
      clips.push({ kind: 'blank', inPoint: 0, outPoint: blankDuration, speed: 1, volume: 0 });
      continue;
    }
...
async function renderProject({ project, assetRows, outputPath, options = {}, onProgress, signalRef }) {
  ...
  clips.forEach((clip, index) => {
    ...
    if (clip.kind === 'blank') {
      filters.push(`[${index}:v]trim=duration=${renderedDuration.toFixed(3)},setpts=PTS-STARTPTS,setsar=1,fps=${canvas.fps},format=yuv420p[v${index}]`);
    } else {
      filters.push(
        ...
        `pad=${canvas.width}:${canvas.height}:(ow-iw)/2:(oh-ih)/2:black,setsar=1,fps=${canvas.fps},format=yuv420p[v${index}]`
      );
    }
  ...
  filters.push(`${concatInputs.join('')}concat=n=${clips.length}:v=1:a=1[vbase][amain]`);
