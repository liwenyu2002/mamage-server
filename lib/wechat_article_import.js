...
const SVG_ATTR_WHITELIST = new Set([
  ...
  // <animateTransform type="translate|scale|rotate|…"> 的 type：区分变换种类，缺了整段轮播/位移动画失效
  // （3t9ztRj… 那篇横滑轮播就是靠 animateTransform type=translate 平移一条 8 屏胶片）。仅 svg 子树生效。
  'type',
  // <animateMotion> 路径运动（另一种轮播/位移做法）的声明式属性，无脚本能力
...
// SVG 子树内含 URL 的属性（伪协议注入面）——只对这些属性查 javascript:/vbscript:。
const SVG_URL_ATTRS = /^(href|xlink:href|src|values|from|to|by)$/i;
function stripAttrsToWhitelist($, $root) {
  $root.find('*').addBack().each((_, node) => {
    ...
    Object.keys(node.attribs).forEach((name) => {
      ...
      if (svgScope) {
        // ★ 通用逆向关键：svg 子树改「反白名单」。秀米/135/自研的动画·滤镜·文字·自定义 data-* 属性无穷无尽，
        // 正白名单永远补不全（type/font-size/enable-background/t-brush-cr/data-cacheurl… 每篇都冒新的）。
        // svg 是声明式、无脚本执行面，唯二注入面是 on* 事件与 URL 属性里的 js/vbscript 伪协议——只删这两类，
        // 其余整树原样保留。foreignObject 内嵌的 HTML 由客户端 DOMPurify 递归按 HTML 规则再洗一遍兜底。
        if (/^on/i.test(key)) { delete node.attribs[name]; return; }
        if (SVG_URL_ATTRS.test(key) && /(javascript|vbscript)\s*:/i.test(node.attribs[name] || '')) { delete node.attribs[name]; return; }
        return;
      }
...
function proxifyBackgroundImages($, $root) {
  ...
  // svg <image> 的 href/xlink:href 也走同源代理（否则 background 之外的矢量内嵌图仍会命中防盗链水印）。
  $root.find('image').each((_, el) => {
    const $el = $(el);
    ['href', 'xlink:href'].forEach((a) => {
      const v = $el.attr(a);
      if (!v) return;
      const p = proxifyWeChatImageUrl(v);
      if (p !== v) $el.attr(a, p);
    });
  });
}

// 微信/秀米懒加载背景图：真正的图 URL 放在 data-lazy-bgimg（或 data-bgimg/data-bg），页面上可见的
// background-image 只是 1px 占位，微信前端 JS 在进入视口时才把真链换进去。属性白名单会删掉 data-*，
// 导致这些图凭空消失——Color Walk 那类「foreignObject 里嵌 <svg data-lazy-bgimg=彩色层>」的点击上色
// 就是这样丢掉全部彩色图层的。必须在白名单剥离之前，把 data-lazy-bgimg 提升进 inline background-image
// （随后 proxifyBackgroundImages 会把它一并改写成同源代理，预览端也能直接加载、无需再跑懒加载 JS）。
// 各家编辑器懒加载背景图的 data-* 属性名（秀米/135/微信各不同）——真实图 URL 藏在这里，可见的
// background-image 只是占位。属性白名单已改反白名单会保留它们，但预览端不跑各家懒加载 JS，仍需在此
// 主动把它们提升进真实 inline background-image（随后统一代理）。
const LAZY_BG_ATTR_NAMES = ['data-lazy-bgimg', 'data-bgimg', 'data-bg', 'data-backurl', 'data-cacheurl', 'data-original-bgimg', 'data-background-image'];
// svg <image>/<img> 懒加载真实 URL 的 data-* 属性名
const LAZY_SRC_ATTR_NAMES = ['data-src', 'data-original', 'data-backurl', 'data-cacheurl', 'data-url'];
function firstHttpAttr($el, names) {
  for (const n of names) {
    const v = String($el.attr(n) || '').trim();
    if (/^https?:\/\//i.test(v)) return v;
  }
  return '';
}
function promoteLazyBackgroundImages($, $root) {
  $root.find('*').addBack().each((_, el) => {
    const $el = $(el);
    const tag = String(el.tagName || el.name || '').toLowerCase();
    // svg <image>：真实图常在 data-src/data-cacheurl 等，占位在 href；提升为真实 href
    if (tag === 'image') {
      const real = firstHttpAttr($el, LAZY_SRC_ATTR_NAMES);
      if (real) { $el.attr('href', real); $el.removeAttr('xlink:href'); }
      return;
    }
    // 背景承载元素：把懒加载 URL 提升进 background-image
    const lazy = firstHttpAttr($el, LAZY_BG_ATTR_NAMES);
    if (!lazy) return;
    const st = String($el.attr('style') || '');
    const next = /background-image\s*:/i.test(st)
      ? st.replace(/background-image\s*:[^;]*/i, `background-image: url(${lazy})`)
      : `${st}${st && !/;\s*$/.test(st) ? '; ' : ''}background-image: url(${lazy})`;
    $el.attr('style', next);
  });
}
...
function cleanContainer($, $container) {
  ...
  promoteLazyBackgroundImages($, $clone); // 必须早于属性白名单：data-lazy-bgimg 随后会被白名单删除
