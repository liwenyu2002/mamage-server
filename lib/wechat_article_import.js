...
function promoteLazyBackgroundImages($, $root) {
  $root.find('[data-lazy-bgimg], [data-bgimg], [data-bg]').addBack('[data-lazy-bgimg], [data-bgimg], [data-bg]').each((_, el) => {
    const $el = $(el);
    const lazy = String($el.attr('data-lazy-bgimg') || $el.attr('data-bgimg') || $el.attr('data-bg') || '').trim();
    if (!lazy || !/^https?:\/\//i.test(lazy)) return;
    const st = String($el.attr('style') || '');
    // 覆盖既有 background-image（多为 1px 占位）；没有就追加。仅动 background-image 一条声明。
