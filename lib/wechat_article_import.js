...
function normalizeImages($, $root) {
  $root.find('img').each((_, img) => {
    ...
    // 填充容器型图片（style 里 height 非 auto，典型是圆形头像：方形框 + width:100%;height:100% 的 img）
    // 默认 object-fit:fill 会把图拉伸铺满、变形（人被拉宽）。补 object-fit:cover 改为等比裁切填充，保住比例。
    // 只针对"有固定/百分比高度"的图，普通 height:auto 全宽图不动（object-fit 对它本就是无操作）。
