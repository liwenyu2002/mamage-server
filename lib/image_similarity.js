...
// the `python` command, or set IMAGE_SIMILARITY_PYTHON_PATH/PYTHON_PATH.
async function encodeImageFromBuffer(buf) {
    return runWithEncodeLimit(async () => {
        ...
        const python = process.env.IMAGE_SIMILARITY_PYTHON_PATH || process.env.PYTHON_PATH || 'python';
...
async function saveEmbedding(photoId, embedding, modelName) {
    ...
    try {
        ...
        await pool.query('DELETE FROM ai_image_embeddings WHERE photo_id = ? AND model_name = ?', [photoId, m]);
