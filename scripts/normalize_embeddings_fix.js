...
async function main() {
    try {
        console.log('[normalize_embeddings_fix] scanning ai_image_embeddings...');
        const [rows] = await pool.query('SELECT id, embedding FROM ai_image_embeddings');
        console.log('[normalize_embeddings_fix] rows:', rows.length);
        ...
        for (const r of rows) {
            ...
            try {
                ...
                console.warn('[normalize_embeddings_fix] skip id', r.id, e && e.message ? e.message : e);
                skipped++;
            }
        }
        console.log('[normalize_embeddings_fix] done updated=%d skipped=%d', updated, skipped);
        process.exit(0);
    } catch (e) {
        console.error('[normalize_embeddings_fix] error', e && e.stack ? e.stack : e);
        process.exit(1);
    }
}
